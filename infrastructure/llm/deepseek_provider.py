"""
deepseek_provider.py - DeepSeek LLM实现
...
class DeepSeekProvider(BaseLLMProvider):
    ...
    def __init__(
        ...
        参数:
            model_name: 模型名称 (deepseek-chat, deepseek-coder)
            api_key: API密钥
            base_url: API基础URL
            timeout: 请求超时时间
        """
        ...
        logger.info(f"DeepSeek提供者初始化完成，模型: {model_name}")
    ...
    async def generate(
        ...
        if not self.validate_config():
            ...
            return "抱歉，DeepSeek服务未配置，请检查API Key设置。"
        ...
        try:
            ...
            logger.debug(f"DeepSeek生成成功，输入长度: {len(prompt)}, 输出长度: {len(result)}")
            return result
    ...
    async def stream_generate(
        ...
        if not self.validate_config():
            logger.error("DeepSeek API Key未配置")
            yield "抱歉，DeepSeek服务未配置，请检查API Key设置。"
            return
